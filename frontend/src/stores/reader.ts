...
interface SpeechConfig {
  ...
  speakerConfigs?: Record<string, { preloadCount: number, gapReduction: number, speechRate: number }>
}
...
export const useReaderStore = defineStore('reader', () => {
  ...
  function setSpeechRate(rate: number) {
    ...
    if (speakerId) {
      ...
      if (!speechConfig.speakerConfigs[speakerId]) {
        speechConfig.speakerConfigs[speakerId] = { preloadCount: 3, gapReduction: 0, speechRate: rate }
      }
      speechConfig.speakerConfigs[speakerId].speechRate = rate
    }
    clearPreloadedOpenAIAudio()
    saveSpeechConfig()
  ...
  function syncTTSConfigToNative() {
    const speakerId = speechConfig.provider === 'http' ? speechConfig.httpTtsActiveId : speechConfig.openaiVoice
    if (speakerId) {
  ...
  function startTTS(text?: string, options: TTSOptions & { startIndex?: number } = {}, interruptCurrent = true) {
    ...
    if (invokeTTS('play', {
      text: content.value.replace(/<[^>]+>/g, '').trim(),
      bookUrl: book.value?.bookUrl,
